...
| `single_sc_rad_boundary.yaml` | **DLF + Boundary + sm_harvest（Z 三代 A/B 的 sigmoid 基準）** | 在 `single_sc_rad_dlf` 上 `loss.boundary` ＋ `surrogate.pretrained: sm_harvest.pth`（⚠ 與 dlf 多了 SM-init 變因；old_sm≈隨機故統一換） | （新增；亦為 `_boundary_mirror`/`_zbatch` 的 sigmoid 基準；**需正式機 HFSS**） |
| `single_sc_rad_boundary_mirror.yaml` | **最強 recipe + 鏡像對稱** | 在 `single_sc_rad_boundary` 上 `generator: mirror`（唯一變因） | （新增，三代生成器 A/B 之一；對標 `single_sc_rad_boundary`(sigmoid)；**需正式機 HFSS**） |
| `single_sc_rad_zbatch.yaml` | **最強 recipe + 同批多候選（Z）** | 在 `single_sc_rad_boundary` 上 `generator: batch_latent`（唯一變因；同批 K 候選、SM 評分選最佳送 HFSS、mean-over-K 反傳；旋鈕 `num_candidates/sigma/sigma_min` + 選用 `selection:` 區段） | （新增，三代生成器 A/B：Z vs 對稱 vs sigmoid 基準，同 recipe 比；**需正式機 HFSS**） |
| `single_sc_rad_boundary_multiscale.yaml` | **最強 recipe + 多尺度淺層生成器** | 在 `single_sc_rad_boundary` 上 `generator: multiscale`（唯一變因；1×1/5×5/13×13 上採樣相加、淺層少參數；cold-start「generator=重參數化/先驗」+ 按尺度退火） | （新增；下一輪 generator-prior A/B：sigmoid vs mirror vs multiscale，同 sm_harvest recipe；**需正式機 HFSS**） |
...
- **SM 初始化統一 sm_harvest（下一輪）**：`single_sc_rad_boundary` / `_boundary_mirror` / `_zbatch` 三支 `surrogate.pretrained` 已改 `sm_harvest.pth`（old_sm≈隨機）。⚠ 同名 config 在既有結果夾會「續跑」載舊 checkpoint → **要吃到新 SM 初始化得用全新結果夾**（清掉舊夾或換 `name`）。
