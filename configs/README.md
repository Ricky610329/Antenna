...
- **`direct` generator 對照組：量「架構先驗值多少」（待辦，純消融）**：per-task 單 target 下 generator 不是學「spec→pattern」映射、而是把這一張 pattern **重參數化**（≈deep image prior）；架構＝隱式先驗。極端 baseline = 完全不要 generator，`logits = nn.Parameter(out_dim)`、forward 只過 `BiScaleNorm`（~10 行 nn.Module + zoo 加一行，golden 隔離）。意義：架起一條**對偶**——「generator 架構先驗（multiscale 連通/mirror 對稱，隱式、免調權重）」⇄「顯式正則 loss（SC/island/boundary，顯式、要調係數）」，兩者都在「限制搜尋空間、別鑽 SM 盲區」。direct 是 multiscale↔direct 先驗光譜的「最無先驗」端：若 `direct + SC/boundary` 追平 `multiscale` → 證明 generator 可拆、用顯式 loss 替代（輕量化勝利）；追不平 → 架構先驗有 loss 補不回的價值。⚠ 純 direct 無先驗 → 易得破碎/棋盤解專鑽 SM 盲區（正是要量的）。amortization（跨 task「規格→好起點」泛化）才是 generator 不可替代之處，但現行 per-task 定位用不到。
