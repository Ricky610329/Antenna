...
| `single_r5_explore.yaml` | **【Round 5 E】探索臂 + 滑動視窗 SM** | = `single_r4_explore` 唯一變因 `sm_train.mode: adaptive→adaptive_window`（window 旋鈕 + `replay_size 512`） | （R4 實測深度欠訓：每輪訓完 elite fit_loss 仍 ~8-11、adaptive 探測自鎖 target 3-5；滑動視窗＝訓到視窗頂+log2 階梯快照+argmin 貼邊×2/÷2（Ricky 設計）；對照 R4 E；判準 fit_loss 壓到 ~1-3 + sm_gap/bias 降 + trust 升；**需正式機 HFSS**） |
| `single_r5_dip.yaml` | **【Round 5 D】DIP 臂 + 滑動視窗 SM** | = `single_r4_dip` 唯一變因 `adaptive→adaptive_window`（+ `replay_size 512`） | （同 R5 E 修法套在 sigmoid 臂——sm_bias ~8-10 最需要把 SM 訓起來；對照 R4 D；**需正式機 HFSS**） |
| `single_r5_dip_explore.yaml` | **【Round 5 E+D】DIP+探索 + 滑動視窗 SM** | = `single_r4_dip_explore` 唯一變因 `adaptive→adaptive_window`（+ `replay_size 512`） | （R4 此臂破專案紀錄 -2.89@154（探索撞到）→ 看 SM 訓起來能否把「撞到」變「開採」；對照 R4 E+D；**需正式機 HFSS**） |
