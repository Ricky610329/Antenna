...
## 🔜 候選 / 待排
- **🔜 已排定下一批:probes 批（~24 筆,三因果探針）**——①全對稱冠軍×8（對稱度=rad 旋鈕?）
  ②搭橋測試×8（懸浮件功能性,孤島論點壓陣）③圖4-4/t07 構造化×6（第二山頭?）;判準已寫死於
  scratch「下一批規格」塊;觸發=ref3/wide 收檔任一機空;需新算子 add_bridge。
- **底緣邊緣精修臂（壓帶外機理）**：帶外承重圖定位元凶=主件底緣 → 只翻底緣 1-2px 換選擇性;
  觸發=probes 批之後。詳見 scratch 2026-07-07 晚塊。
