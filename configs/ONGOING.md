...
### Round 33 — 反王朝結構輪（🔵 **running**,2026-07-17 晚開輪;Ricky「輪次持續開」+表型黑名單口徑拍板;跨週末）→ [round-33](../docs/log/round-33-anti-dynasty.md)
- **主軸**：表型黑名單（底1大+上2中;gen 雙過濾+select 罰 +2.0,錨定臂豁免）＋L-RADGATE 六錨
  （rad 閘攻堅）＋CNN 混合鍵（記錄版）＋B 泵續投（selfgen 三錨集中）。
- 主判準:批內王朝結構佔比 <40%（基線 81%）;rad 閘=同框∧rad≥−1 ≥1/批;判準寫死見 round-33 §1。
- b1 待 v45 三件套落地即發;明 17:00 NAS 關機,餘批週一（重啟指令見 round-33 §3）。
- 擇機:帶內 0.61 rad 修復;novelty 鍵擴 PCA 距離（R34 候選）。
