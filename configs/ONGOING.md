...
### Round 29 — G 臂主力輪（🔵 **開輪中**,2026-07-15;Ricky 2026-07-14 拍板「R28 收完就開,G 臂多跑一點,其他啟發式/探索再降比例」）
- **配額**（開輪判準寫死時定稿）：**G 76（~50%,絕對主力）**/M 14（前瞻對照母體）/O 12（梯子房租）/
  I 14/D 12/W 10/C 6/S 6/**F 0（退役——修復精神由 Y/G 繼承）**。
- **G 臂內部 mix**：random-init 自由帶（**多樣性主力**）~28/half 手術帶（得分主力,rad 目標）~24/
  冠軍中帶 ~12/**超規格帶外期望**（oob 目標 ≤6=低側資料泵）~12;每帶記 pred vs realized=
  adversarial 率=SM 盲區地圖。
- **承重圖=G 知情約束**（R28 正資產）:凍命脈塊、自由塊當畫布、rad 期望 +2 內誠實（次可加折扣）。
- 實作:sm_invert 加 `gen`（staging 夾）→ select-r29 G 臂讀入走標準 manifest/查重;工具已備（d599d14）。
- lo-active 配額推 ~50%（decisions 反自餵②）;rad-key 保留（再 <0.3=連兩批→退鍵）;216 三機體制。
