...
**⚠ 週末排程（2026-07-17 Ricky 通知）**：**07-18（六）17:00 NAS 關機;07-19（日）停電**——
批次線週末全停。今晚（07-17）R32b3 收檔+收輪照常;收輪後 selfgen 自轉到週六 17:00。
**週一恢復清單**:Ricky 開 NAS+三台跑 worker → Claude 跑恢復檢查（probe 三台+jobs_state 清殘留
+`/takeover`）→ R33 開輪（工具鏈已備,見下方 🔜）。
...
- 進度:b1 海峽落點 92% 幾何過/電性 0;b2 X 判死+影子 ρ 軸連兩批碾壓;**b3（末批,15:31 發車）
  =B 層 wm 泵首航**（六錨血統 A/B,判準見 round-32 §1 修訂）。工作模型 v44 三件套。

### 🔜 R33 候選（觸發:週一恢復後開輪;定案=decisions「王朝重定義」「影子 CNN 對決」兩節）
- **反王朝結構輪（主軸候選）**：select 生成端結構判過濾/降權（王朝表型=下1大+上2中;
  **中繼家族+lo 實測<0 豁免**）＋恆溫器加結構多樣性軸＋PCA 網格佔據率進判讀＋novelty 鍵擴
  PCA 距離——Ricky 期望「PCA 分布更均勻,多種 diversity 檢查都要均勻」;全史八成困王朝結構=空間大。
- **混合鍵**：select 排序摻影子 CNN（ρ 連兩批 3 倍）、絕對值/LCB 留 MLP;影子三尺帳續記。
- **B 泵複盤**：b3 首航結果決定續投或轉法;selfgen 貼線簇=非王朝可用帶外候選池（scratch 有記）。
- 分布圖工具化 ✅（figs/pattern_map＋wm_terrain,2026-07-17）;帶內 0.61 rad 修復候選（擇機）。
