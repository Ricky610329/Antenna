...
- 🔵 **R72 裙擺攻堅+別峰首發**(待開;主算子=雜交〔R71 選型定案〕;器官庫=G_21 阻帶/
  X_34 帶內/王 m3;band-edge 頻點=25/25.5/29.5/30/30.5/31;SM v5 重考先行〔鍋過 2,000〕)。
...
- 🔜 **single 線王冠三檢回補**(Ricky 2026-08-13 記錄;觸發=single 線重啟或交付前):
  對現役冠軍(margin 王 c48nq1p05_16 +0.79/rad 王/左側 c8trip03_01)補跑 dual 今日
  制度化的三檢②③——橋依賴三態(fill/切角/裸)+跨尺度括號(從未做過)。
  why:single 量測幾何=隱式 0.01 微橋,出圖幾何=0.075 菱形,R54 判「稅≲噪音」但
  只驗過 ~10 親;**dual 今日實測 0.01 vs 0.075 在王身上差 3dB**——「稅≲噪音」可能
  是個體依賴,冠軍們值得逐一重驗。順帶議題:single 要不要也把菱形升常態儀器(量測
  與出圖同幾何,消除 dual 剛消滅的那種錯位)。
- 🔜 **E2E 純自產血統線**(Ricky 2026-08-13:「很難說我們可以 end to end」;
  觸發=autd2 累積 ~3,000 筆,估 3 天):種只准自產(autd2/B/C 後代,lineage=own 逐筆標記),
  同引擎(選拔+標準橋+開採)平行磨;論文雙線圖=harvest 種線 vs 純自產線——追平=
  end-to-end 主張掙到,追不平=誠實收斂為「搜尋+精煉引擎」。基線帳已開:R71 hc 對照臂
  (均勻隨機 harvest+橋)每批量測選拔/開採增益。
- 🔜 **別峰臂常設**(Ricky 2026-08-14:「圖相似性很高,找別的山頭」;R72 起):每批保留
  15-20 席給「距王 Hamming≥80」候選——來源=①SM v5 過閘後篩全 harvest+橋(一萬顆
  新尺下未看過)②autd2 隨機臂遠親前段③乾淨生成語言。判準獨立:遠親達 −5 級=
  第二座山立案開王朝。⚠R67 遠親全滅結論=幻影儀器量的,已作廢,mfg 代真正未試。
