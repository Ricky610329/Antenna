...
### Round 26 — 帶外前瞻復活驗證（🔵 **running**,2026-07-14 開輪）→ [round-26](../docs/log/round-26-oob-foresight.md)
- 主假設=**SM 帶外排序復活了嗎**（R25b3 oob 前瞻 ρ +0.577 p=0.008,長期基線 <0.1）;判準=3 批 M 臂
  oob ρ 中位 ≥0.3 → 帶外導引升級議程。
- **退 rad 鍵**（R25 觸發連兩批 <0.3;--rad-head 續記,復鍵=續記前瞻連兩批 ≥0.3）;I 18→**26**（61% 爆發加碼）;
  **F 縮編續 1 期 12 席**（預設方案,趨勢正——Ricky 可否決）;D 8=第二期判決批（min sel 89.8/93.0→b1 判）;≤3 批。
- 發車=`select-r26 --batch N --sm sm_reanchor25+ --novelty`;每批 /batch-cycle。
...
- **圖 4-4「骨架+網布」塊級 variation（Ricky 2026-07-14 提問促成）**：池頂家族實測=8-10 密度分塊
  （σ0.8,載 66% 金屬）＋網布,且 **t03/t09/n09/p00 共享同一副骨架**——同骨架重抽塵埃（H1）/全凝聚
  vs 半凝聚（H2,拆「低側住骨架還是網布」）/塊級算子（H3,F 臂第三期武器候選）;~25 筆=1/6 批。
  **觸發=R27 開輪候選;Ricky 點名可提前**。詳 scratch 2026-07-14 塊。
