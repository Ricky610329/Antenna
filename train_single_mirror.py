...
from antenna.models import MirrorCVAE, Models, OldGEN
...
RESULT_PATH, CONTINUE_RUN = get_result_path(
    "[Patch Single][{device}] MirrorCVAE_dual", rootdir=ROOTDIR, generate_code=__file__, enable_exception_handler=True
)
...
while epoch < config.epochs + 1:
    ...
    with Figure(
        ...
        fig_response_s11.set_title("S11", fontsize=20)
        ...
        fig_response_gain.set_title("Gain", fontsize=20)
        ...
        for n, result in enumerate(all_results, 2):
            ...
            time_str: str = "No simulation" if result["time"] is None else f"time: {int(result['time'])}"
            loss_title = (
                f"{result['fake_loss']:.2f}"
                if result["real_loss"] is None
                else f"{result['fake_loss']:.2f} -> {result['real_loss']:.2f}"
            )
    ...
    exe_time = 0
    logger.info(
        f"End {epoch} of {config.epochs}, Loss: {TEMP('real_loss'):4f}, Time: {sum([r['time'] for r in all_results]) / 60} m, jump: {jump}"
    )
