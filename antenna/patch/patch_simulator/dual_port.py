...
class DualPortSimulator(PatchSimulator):
    ...
    def __call__(self, pixel_matrix: Tensor):
        ...
        oModule.CreateReport(
            ...
            ["Domain:=", "Sweep"],
            ["Freq:=", ["All"]],
            ["X Component:=", "Freq", "Y Component:=", ["dB(S(2,1))"]],
        )
        ...
        oModule.CreateReport(
            ...
            ["Domain:=", "Sweep"],
            ["Freq:=", ["All"]],
            ["X Component:=", "Freq", "Y Component:=", ["dB(S(2,2))"]],
        )
        ...
        oModule.ExportToFile(
            "S Parameter Plot 2", self.path_result.joinpath(f"NN_patch_Sparameter_{self.num}_S21.csv"), False
        )
        oModule.ExportToFile(
            "S Parameter Plot 3", self.path_result.joinpath(f"NN_patch_Sparameter_{self.num}_S22.csv"), False
        )
        ...
        np.append(np.append(np.append(full_output, S11.to_numpy()), S21.to_numpy()), S22.to_numpy())
        # breakpoint()
        return _result
