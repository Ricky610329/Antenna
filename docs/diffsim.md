...
**★ 若要重啟，病因已收斂到一個目標**（analysis-10 §13–§18）：

```
✅ 已驗證正確：核（empymod 四管道）／自項（真積分對實測）／遠場絕對量級
                （單一電流元對解析解 0.998）／共振頻率（比閉式低 8–9%，四尺寸一致）
                ／Zin 對埠寬度不變（同一個 gap 的切片，是對的物理）
✅ 已確認有影響但不是全部：離散化（<10 格時幾乎沒有輻射；≥15 格飽和在 0.33）
    ↓
❌ **未解：輻射電阻低 ~7 倍**
   = Re(Zin) 低 ~3.3 倍（178 Ω vs Balanis 完整式 534 Ω）× η 只有 0.43（應 0.72–0.77）
   ⚠ 兩者**可能是同一件事**：模型的 R_sw ≈ 101 Ω 與文獻反推的 ~134 Ω 其實很接近，
     所以偏差幾乎全在 R_rad
    ↓ |Γ|≈1 → S11 很淺 → 谷很平 → 谷位置 ρ=+0.078 → wm／選批跟著壞
```

**下一個人只需要找一個東西：為什麼輻射電阻低 ~7 倍。**
遠場對「給定電流」的積分已驗證正確 ⇒ **嫌疑在電流分佈本身**。
對照工具已建好（`scratchpad/rrad_check.py`、`disc_conv.py`＋Balanis 完整式含 G₁₂），可直接重跑。

⚠ **兩次「基準本身錯了」的教訓**（analysis-10 §17.2）：我用文獻閉式當 ground truth 時
**兩度取了簡化式**——先漏共振頻率的邊緣延伸 ΔL（把「低 8–9%」誤判成「低 20%」），
再漏輻射電阻的互導納 G₁₂（把「低 3.3 倍」誤判成「低 4.8～14 倍」）。
簡化式的省略項在我們的參數區間（`W/λ₀ ≈ 0.22–0.47`）**不小**。**基準也要被審查。**
