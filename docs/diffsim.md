...
**★ 出貨組態＝`l3fld`**(再加對角連通;`build_l2` 與所有 CLI 的預設,2026-08-04 §51.5 定案)。
同批 fit clean n=680 的 `a_quad` 2×2 消融把 §49.5 的「ρ 暴漲而命中率崩掉」歸因完畢:
**不是對角的錯,是 `a_quad=4` 的錯**——`l3fl` q1 26.7%／q4 **31.7%**(無對角時更準的求積如預期更好)
vs `l3fld` q1 **55.0%**／q4 **15.0%** ⇒ 交互作用專屬於「準求積 × 對角基底」。
⚠ 代價:出貨走的 `a_quad=1` 有一個**直接數值積分證實的錯誤**(對角×軸向共用 `cell_a` 互耦高估 **69%**),
假說是它剛好在模擬未建模的 10µm 頸縮(§51.4,待驗)
