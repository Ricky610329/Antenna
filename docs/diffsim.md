...
證據是**無因次**的（不需要 `Z_c`、不需要擬合）：同一個模態振幅下的饋入電流比
`I_feed/I_max`，delta-gap 打邊 ÷ 接 9mm 饋線 = **埠因子 1.30–1.60**
（analysis-10 §41.1 修正——原本寫 1.71，那是對 (a)(b) 用了兩種不同頻率準則算出來的，
對稱協定給 1.30–1.60；**方向保留、「硬下界」措辭撤回**）。
⭐ **更硬的一條是共振頻率**（外部錨、與 Balanis 的輻射電導無關）：
`l3` 對閉式解 **−7~−8%**，饋線埠 `l3fl` **−1.5~−3%**
—— 而 §11 早就把病定位成「共振頻率完全抓不到」。
⚠ 完整的阻抗帳**目前不平**：用實測的 `Z_c ≈ 50 Ω` 重算，`Re(Zin)/Balanis` 是 0.76–0.85，
它落在哪個靶上取決於「源端洩漏 40%」是否為真（analysis-10 §40.4/§41.5，未定案）。
