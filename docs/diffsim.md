...
**★★★ 根因已確認**（analysis-10 §19）：**rooftop 基底表達不出邊緣的電流奇異性**。

導體邊緣有 edge condition `J ∝ 1/√d`，而 rooftop 是線性基底。實測：
`Re(Zin)/閉式` 的比值隨格數 **7→23 從 0.192 單調升到 0.406**，
收斂形式擬合 **`1/√N` 最吻合**（RMS 0.064，p=1 是 0.243、p=2 是 0.835）
—— **`1/√N` 正是「基底表達不出奇異性」的指紋**。
要到比值 0.9 需要 **629 格（dx = 0.005 mm、約 79 萬未知數）**，完全不可行。

⇒ **這是模型類別的限制，不是 bug。** 而且它與 L2/L3 便宜的原因（平移不變 ⇒ 查表）
**互為代價**：均勻網格＋統一基底才能查表，而那正好表達不出邊緣。

**已驗證的旁證**：
- 橫向剖面的「邊緣/中央」峰值比：模型 **2.40** vs 解析 `1/√(d(1−d))` 的 **2.96**
- 由 `1/√N` 擬合**獨立預測**的校正因子 `k ≈ 2.55`，
  與「讓 min-S11 對上真值絕對量級」的 k（2.5–3.0）**吻合**（真值中位 −16.38 dB）

⚠ **但修對絕對量級之後 rank 反而崩**（ρ +0.366 → +0.066）：`Re(Zin)` 中位 21 Ω，
×2.5 ≈ Z₀ = 50 Ω，**匹配點附近 `|Γ|` 對誤差極度敏感**。
現況 rank 還可以，是因為 `|Γ|≈1` 的平台區把誤差壓平了。
⇒ **「量級對」與「排序對」衝突**，來源是比值不是常數（0.19–0.41）。

⭐ **但 k=2 已有真實改善：命中率 12% → 20%（隨機 10%）** ——
diffsim 在正片域第一次真正勝過隨機。⇒ 下一步＝**學 pattern-specific 的 k**
（低維、有物理先驗 `k ≈ 1/(1−C/√N_eff)`、與邊緣幾何相關），
而不是 §10 那種「把 34 維輸出丟給 head」的黑盒疊加。

❌ **已試過並失敗**：解析的加權基底（`f_j = w_j·rooftop_j`）——
那在 Galerkin 下是**恆等變換**（`J = W(WZW)⁻¹WV = I`），改振幅不改變函數空間。
要表達 `1/√d` 必須改**格內形狀函數** ⇒ Z 的積分核要重算，且會破壞平移不變性。
