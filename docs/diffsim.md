...
**★ 若要重啟，病因已收斂到一個目標**（analysis-10 §13–§16）：

```
共振頻率  ✅ 準（比閉式低 8–9%，四個尺寸一致）
遠場計算  ✅ 準（單一電流元對解析解 0.998）
核 / 自項 ✅ 準（empymod 四管道、真積分對實測）
    ↓
**Re(Zin) 低估 ~4.8 倍**（共振點 178 Ω vs Balanis 積分式 862 Ω）  ← 唯一的病
    ↓ |Γ|≈1 → S11 很淺 → 谷很平 → 谷位置 ρ=+0.078 → wm/選批跟著壞
```

已排除的候選：遠場係數／遠場取樣／閉式解適用性／介電損耗／共振頻率／埠寬度／Stage 1 副作用。
**最具體的入口**：`Re(Zin)` **幾乎不隨驅動寬度變**（1 格 179.9 Ω vs 25 格 170.7 Ω）——
健康的 MoM 不該如此。**驗收指標**＝共振點 `Re(Zin)` 對上 Balanis 積分式（比值 0.05–0.09 → O(1)）。
