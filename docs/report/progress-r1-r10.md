...
**R4 值得單獨講清楚（線上線的高峰）**——它同時是最好成績與最誠實的歸因案例：

1. **產品成果**：E+D 臂 **−2.89@154＝線上線史上紀錄**，勝 R3 同臂 +2.80dB。躍遷形狀很有代表性：
   −7.60@76 → −7.13@153 → **−2.89@154**——長停滯後單步 +4.24dB，是探索撞進質變區，不是漸進爬升。
   這個數字此後保持到線上線收束，等於「梯度範式在本問題的天花板」的實測定位。
2. **誠實歸因**：破紀錄時 trust 仍鎖在 0.05——**是探索的功勞，不是主假設（自適應訓練→信任→利用）生效**；
   而且 `mode:adaptive` 一次換了兩個機制（訓練量自適應＋移除單筆過擬合反模式），結論只能寫
   「SM 更新規則整包換」。主假設實際上**未驗證**：adaptive v1 的探測只看「目前訓練量以下」
   （結構性自鎖，target 卡 3-5、探測曲線 80-100% 平坦），SM 始終深度欠訓（fit_loss 仍 7.7-10.6）。
3. **方法論價值**：這兩個實錘診斷（深度欠訓＋探測自鎖）直接催生 R5 滑動視窗設計（訓到視窗頂、
   探測永遠看得到上方），R5 才把 SM 真的訓起來（fit_loss 8-11 → 0.45）。R3→R4→R5 是
   「一輪消除一個瓶頸」的完整診斷鏈——對論文而言，這條鏈比任何單輪成敗都值錢。

> **▍一句話重點**：五輪修了五個瓶頸，SM 從欠訓修到訓好，但 best-wm 天花板停在 −2.89（R4，且那次躍遷
> 是探索的功勞）——問題不在訓練，在**範式**。這是 R6 轉向的動機。
