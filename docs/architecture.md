...
| v12 (15×15, **WideGumbelSigmoidGEN** clamp [-20,20]) | T_0=200, scheduler decoupled | 20 | 0.10 | TBD | **7.15** | **wide clamp 沒解決 collapse** — best pattern 仍 100% on、logits 實際只用 [0.44, 0.57] 範圍、響應跟 V7 100% 一致；loss 反更差（同 SA dynamics 但梯度差異）。**確認問題不在 clamp 邊界**。 |
