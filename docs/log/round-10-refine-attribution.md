...
- **狀態**: archived（2026-07-07 收檔——八冠軍 certified,判準全格達成）
...
**掃頻法交叉驗證（2026-07-06 晚,`--sweep Discrete`）**：9 個關鍵 pattern（w17/s05/g24/g15/y05/e39/t00/a00/p03）
Interpolating vs Discrete 逐點硬解——**margin 與全響應曲線逐位相同（9/9,0.00e+00）**,耗時同級
（~100s/筆,大頭是 28GHz 自適應網格、兩法共用;「Discrete 慢很多」在 17 點規模不成立）→
**Interpolating 擬合誤差 ≈0,快速掃頻可信任**;九筆 certified 值蓋硬解章（w17 −0.06 第四脈絡確認）。
過程另揪出並修復第二個批次線污染源（跨批共用工作目錄+同名 CSV 殘留,雙保險 dd38a64）。
+0.48 假象全案閉合:唯一元凶=萃取對位 bug（已修 8079c56）,掃頻/HFSS 非決定性/跨機全數排除。
（ref2 過夜批次判讀另補。）

## 5. 結論 (Conclude)

- **產品格完勝**：八個三標全過的 certified 冠軍（best c21_sm +0.20/+0.12;名鑑 `docs/champions.md`）——
  可製造紀錄一週 −2.68 → +0.20。三種出身都上榜（盲掃 3/知情編輯 1/SM 導引 4）＝方法的多腿都在走。
- **知識格**：「對稱化救爛毀好」過因果關;承重圖×2＋wm-rad 張力空間定位;+0.48 假象偵破 → 萃取修復＋
  「紀錄級一律多維驗證」鐵則＋掃頻交叉驗證（快速掃頻無罪）——量測誠信體系成形。
- **能力格**：SM 重錨至作戰區 0.36dB（分區有效性量化）;超參網格證明資料主導;certified 優先去重防毒。
- **教訓**：ref2 首名 b20 是假象冒充（觸發率 ~9%/37+舊碼）——沒有公證制度它就進論文了。
- **促成**：R11（公差穩健化×規則普適性,occl2/tol/ref3）＋規則→generator 工程（R12 前置）＋碩論框架
