...
- **一句話結論 (TL;DR)**: **`w17_k8` 三標全過（wm +0.48/rad +0.26,可製造）＝專案第一筆完勝**；X 臂 4/4 驗證「對稱化救爛毀好」因果規則；承重圖給出 wm/rad 張力的空間定位＋免費編輯區；SM 重錨 1.41 進帶
...
| 判準 | 結果 |
|---|---|
| 產品 | ✅ **完勝——`w17_k8` 三標全過**（wm +0.48、rad +0.26/+1.75、3 組件零粉塵）＝ s05 翻 8px 再對稱化 |
...
（ref2 過夜批次判讀另補；w17 十次公證 `dedust_w17rep_input/` 已備。）
