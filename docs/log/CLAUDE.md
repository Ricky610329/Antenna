...
- **每輪硬上限 3 批**（Ricky 2026-07-13,強化「過厚警訊」為硬規則）：一輪內部**最多 3 批**,
  第 3 批收檔判讀後就 `/close-round` → `/new-round`,不拖第 4 批。輪的邊界是「假設被回答」,
  不是「管線停下來」;換王/公證是事件不是假設,記在當下運行的輪即可。擴散型介入的長 baseline
  評估**跨輪累積**（gain-check L2,見 decisions「探索型介入的評估法」）,不靠單輪撐多批。
...
## Round KPI 五軸面板（2026-07-15 戰略換軸,decisions「戰略換軸」節）

每輪收檔 §4 必貼五軸（analyze batch 自動輸出②③,sm_reanchor 自動記①的 held-out → docs/kpi.csv）：
①**SM 準度（主 KPI）**=held-out 誤差+前瞻 ρ+G 臂 adv 率——輪的成功=此軸有斜率;
②覆蓋=新血/近王/d_dyn;③整體水位=wm 中位/P90+**帕累托前緣增量**+作戰區密度;
④變現=三標/紀錄（有就公證,零推進不焦慮）;⑤健康=error/機器/ikpi。
...
- **機制押注歡迎**:發車後、任何結果回來前可補押注（標日期＋「零結果窗口」聲明）——判準修訂紀律的正向用法（R55 兩例）。
