...
## ★ 更新（2026-07-13）——A 部 variogram 用自家 HFSS 真值重算（侷限 2 解除）

原 A 部吃學長當年 HFSS 回算（侷限 2）。現以 **`python -m script.analyze terrain`**（新子命令）
對自家真值全兩兩配對重算：n=**5,513** 互異 pattern、15.2M 對,全部現行 HFSS 尺。

| Hamming | 1-2 | 3-5 | 6-10 | 11-20 | 21-40 | 41-80 | 81-160 | 161-320 | 321-625 |
|---|---|---|---|---|---|---|---|---|---|
| \|Δwm\| 中位 | **0.17** | 0.21 | 0.44 | 0.87 | 1.19 | 1.59 | 3.02 | 4.69 | 8.37 |

- 長距漸近線（161-625）＝**5.16 dB**;翻 1-2 像素只動 **3%**（學長口徑 12-16%）——
  「地形非抽獎」在自家 HFSS 上**更強**,variogram 單調上升結構完整。
- 有效半徑複驗：41-80 翻轉＝漸近線 31%、81-160＝59% → 「局部引導有效半徑～數十翻轉、
  跨區大跳≈重抽」原結論不變。
- ⚠ 侷限：①短距對幾乎全是王朝刻意變體（選擇偏差比原分析更重——集中在冠軍附近的平坦區,
  3% 是「王朝區局部平滑度」非全域代表值）;②漸近線 5.16 > 學長池 3.79＝自家語料價值跨度更寬
  （de novo 災難～冠軍都入帳）。方向結論穩,數值別跨口徑引用。
