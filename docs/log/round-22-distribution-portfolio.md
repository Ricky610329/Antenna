...
- **狀態**: concluded（2026-07-12 b3 收檔結輪;過厚警訊首例——三假設擠一檔,R23 起守新規）
- **提出 / 開跑 / 結論**: 2026-07-12 / 2026-07-12 / 2026-07-12
- **一句話問題**: R21 的收效遞減是「王朝鄰域這個分布」枯竭所致嗎？——把預算分散到其他分布能否重啟紀錄產出？
- **一句話結論 (TL;DR)**: **能——一天三公證紀錄**（rad 王 m5_054 +0.89/帶外王 o6_001 8.61/帶內 h7_010 +0.46）
  ＋26px 死區告破;C 冷支確立（28→28→34%）、S 槽鏈首批即成立（s8_008 +0.38 三標）、
  Q/H 照判準收臂（低側正式蓋棺）;oob 選鍵紅利耗盡 → 價值軸（sel_score）交棒 R23。
...
- *修訂（2026-07-12 上午,b3 結果未回;Ricky 價值軸定調）*：**過線後帶內餘裕邊際價值≈0,收益軸=帶外**
  （字典序:①三標硬閘 ②oob 升冪;詳 decisions「價值軸修正」）——紀錄優先序即刻生效
  （帶外王=旗艦）,選批鍵 **b4 起**改「pred_wm ≥ buffer → pred_oob 主鍵」;b3 各臂判準照舊。
- *修訂（2026-07-12 09:5x,b3 發車前、b3 結果未回;新臂協議先導）*：**S 槽鏈臂 15/批**——
  證據=H 臂「部分槽 L5-8＝帶內增益旋鈕但付 rad」跨 5/6 錨重現（b1 c25 +0.43/a024 +0.34/r3_001 +0.28,
  b2 g16 +0.46;唯 r2_016 不應答）;假設=槽的 wm 增益可保留、鏈上算子把 rad 修回。
  判準:**三標且 wm≥+0.30＝成立;三標率連兩批 <6%＝收臂**。名額來源=Q 收臂(30)+H 收案(12)釋出。
...
**b2（2026-07-12 09:42 收檔,150/150 零 error——連四批全綠）——判準全面開鍘**：
| 臂 | 三標 | 判準判定 |
|---|---|---|
| O 哨兵 | 3/10（30%） | **前瞻 oob ρ +0.323（p=.042,v13）≥0.3 首次復活 → 照判準回名額**（b3 10→25） |
| M 王朝 | 9/40（22%） | 回穩（b1 10% 是雜訊/稀釋）;v13 wm ρ +0.219 n.s.（逐版波動再現） |
| C 冷支 | 14/50（**28%**） | **連兩批 28% 穩定＝冷支礦確立**（≥王朝級產率）→ b3 再加碼 62 |
| Q 修復 | 0/30（0%） | ✗ **連兩批 <6% → 收臂**;遺產:q7_026_o3_020 wm+0.28/rad −0.04 差 0.04 三標
  （深帶外修復幾乎達陣,線索記 scratch）＋帶內收復 +0.5 級但過不了線 |
| H hslot | 3/12（25%） | **兩批到期 lo 目標 0 筆 → 低側正式收案**（構造法也判死;R17 雙判死補上第三判）;
  遺產:「部分槽 L5-8=帶內旋鈕付 rad」5/6 錨重現（與 R17 整列槽方向**相反**）→ 升格 S 槽鏈臂 |
| W 彩票 | 0/8 | 累計 1/24（死區破了但稀有,照常持有） |
- ★ h7_010_g16 wm +0.46（單次,rad −0.66 非三標）→ n3 公證 ×2（h6_010 前例 −0.08,預期類似）。
- b2 後重錨 v14;b3=O25/M40/C62/S15（新臂先導）/W8,Q/H 名額釋出。

**n3 公證判定（2026-07-12 09:52）——預期落空,紀錄成立**：
- **h7_010_g16 wm +0.46/+0.46 兩重測與原測完全一致 → 帶內紀錄易主（公證 3/3,前任 g14 +0.40）**;
  與 h6_010 假象（−0.08）不同,部分槽 wm 增益「有的真有的假」——公證鐵則兩種都接得住。
- 機理（目檢對比圖）:rad 破口＝**phi90 視軸（θ=0）凹陷**——槽抬帶內同時在視軸挖洞;
  S 臂修復目標具體化＝補中央輻射（R16 中央塊=rad 旋鈕,算子鏈已含 addblock）。
![帶內紀錄易主](assets/round-22/inband_h7010.png)

**b3（2026-07-12 13:10 收檔,150/150——連五批零 error）——S 首批即成立**：
| 臂 | 三標 | 判定 |
|---|---|---|
| O(oob 鍵,25) | 1/25（4%） | oob 選鍵紅利確定耗盡（回名額後仍崩）——R23 換 sel 鍵的依據落死 |
| M(40) | 12/40（30%） | 穩定;前瞻 v14 wm ρ +0.093 死/oob +0.295 邊緣（逐版波動常態） |
| C(62) | **21/62（34%）** | 三批遞增 28→28→34%,**本輪最強產線** |
| S 槽鏈(15) | **5/15（33%）,s8_008_a024 +0.38 三標** | **✓ 首批達成判準（三標且 wm≥0.30）**——
  槽增益保留＋rad 修回（+0.21）,距 margin 紀錄 0.01;⚠ 錨集中 g16 14/15,b 後續要強制錨輪替 |
| W(8) | 0/8 | 累計 1/32 |
- 可用帶外:b3 best 9.16（k8_053）未破 9.09——oob 鍵批打不動這個量,sel 鍵接手（R23 主戰）。
- 紀錄候選:無。/gain-check:L2 邊際 +0.041 dB/百筆;L3 近王 4/推進 6（線還年輕）。

## 5. 結論 (Conclude)
1. **分散假設成立**：預算移出王朝鄰域後**一天三公證紀錄**（rad +0.89/帶外 8.61/帶內 +0.46,
   皆 3/3）＋26px 死區告破——R21 連四批零紀錄的僵局被分布多樣性直接打開。
2. **C 冷支＝新主產線**（28/28/34% 三批遞增,樣本 152）;王朝 M 臂退居對照。
3. **S 槽鏈＝機理轉化的示範**：H 臂死刑中撈出的「部分槽=帶內旋鈕」線索,一批內完成
   「發現→公證（h7_010）→轉化為可用臂（s8_008 三標 +0.38）」全鏈。
4. **判準紀律全程執刀**：Q/H 連兩批線收臂（低側第三判蓋棺）、O 哨兵 ρ 復活回名額又崩、
   h6_010 假象攔截 vs h7_010 真實——兩種結局都被公證鐵則正確分類。
5. **oob 選鍵壽終**（22→3%→回魂 8%→4%）——SM 的 oob 排序只在資料新鮮期有效;
   R23 sel_score（含 rad 頭 ρ=0.498）是結構性替代,不是又一次續命。
6. 制度收穫：tier-2 搶佔（空窗歸零）、sel_score/可用帶外指標、rad 頭（K=16）、
   輪過厚警訊入規、round 號命名規範。

## 6. 後續決策 (Next)
- **R23 接棒**（[round-23](round-23-selectivity-axis.md)）:sel 鍵主戰可用帶外（基線 9.09）;
  S 臂第二批（錨強制輪替）;C 42 續跑;rad 頭進鍵（--rad-key,退鍵線=前瞻連兩批 <0.3）。

## 7. 歸檔指向 (Archive)
- 結果夾: `dataset/dedust_r22b1*..b3*, dedust_r22n1..n3, dedust_r22g1*`;SM v12→v15;rad_head1/2
- 收檔動作: README 索引 ✓/ONGOING 搬家 ✓/champions（三紀錄已隨批更新）✓/memory ✓/scratch 掃 ✓
