...
- **狀態**: concluded（2026-07-16 00:3x 收檔;3 批=硬上限;戰略換軸首輪）
- **提出 / 開跑 / 結論**: 2026-07-14 / 2026-07-15 / 2026-07-16
...
- **一句話結論 (TL;DR)**: **SM 可信半徑實測 d≤25（champ 帶連兩批 realized 三標 ×3=準度變現通道
  打通）;adversarial training 循環成立（ikpi +4.50/吹牛歸零/rad 頭前瞻 +0.621 新高）;
  低側 gap 區破冰（8+ 筆真值踏進 −8~−2 無人區,誤差錨/denovo/碎片三路）;
  margin 王 +0.56（冷支,24hr 連兩王）——戰略換軸首輪全面兌現。**
...
| 2 收檔 | ✅ 20:20（150/150 全批零 error;判讀見 §4 b2;重錨 **v35=密度反權重+分層 held-out 雙首航**〔近1.19/中0.88/遠2.77 基線〕;ikpi I−M **+0.36 成立**;公證 r29n1a=margin 王挑戰 +0.56 發車） |
| 3（r29b3{a-f}）末批 | 🔵 21:1x 發車（v35;**三首航**=①誤差錨外掛 +8〔錯哪補哪〕②G-free 碎片/塊 init〔反塌縮〕③--dyn-frac 0.4〔王朝抽樣降權〕;oobp 錨=原版碎片 t07_top/p00_orig;錨點 748;查重 0×6;恆溫⑥回應=反塌縮 init 已上線,配額不動） |
| 維護日誌 | 21:3x Ricky 三台重開+pull=全配版（selfgen 三分生成 50-150 翻bit/探針/清掃）;216 正式批連三夾滿分（c/e＋b2 兩夾）——磁碟定罪後完全復元 |
| 3 收檔 | ✅ 00:06（07-16;150/150 全批零 error;判讀見 §4 b3;★ 低側 gap 區破冰 8+ 筆;重錨 v36） |
...
3. ★★ **margin 王再易主（公證 3/3,20:37）**：o29b2_011_o26b2_007_o2 三次 bit 級一致
   （wm **+0.56**/rad +0.28/oob 10.9）>前任 s28b3_005 +0.50（在位 10hr）——O 臂 o26b2 系冷支,
   **+0.06 大跳且三軸均衡**（rad 0.28 非貼線）;**冷支連兩王**=反自餵路線連續兌現;
   對比圖 [newking_o29b2_011.png](assets/round-29/newking_o29b2_011.png)。
   帳:margin 王 24hr 內 0.49→0.50→**0.56**（兩易主皆冷支——王朝 c18 系退位）。
...
**b3（2026-07-16 00:06 收檔,150/150 全批零 error;三首航批）**：
| 帶 | |Δwm| 中位（b1→b2→b3） | adv 率 | 三標 |
|---|---|---|---|
| champ | 5.63→4.00→4.79 | 90→75→**74%** | 0→1→**2** |
| free | 10.66→10.66→10.51 | 100% 持平 | 0 |
| surg | 12.20→9.31→11.14 | — | 0 |
| oobp | 12.62→10.47→8.97 | — | 0 |
1. **G 臂連兩批 realized 三標**（b2 1+b3 2=累計 3,全在 champ 帶）——主判準①連續達成;
   champ adv 率收斂至 74%;可信半徑實測維持 d≤25。
2. ★ **低側 gap 區破冰**（Ricky 心病;R27 定案後首批據點）：**8+ 筆真值踏進 −8~−2 無人區**——
   d29b3_006 lo **−8.27**/champ_exking −8.82/**誤差錨子代 i29b3_000_err −6.82**（④機制直接產出）/
   surg_p00h −6.19/碎片 init 系 −4.8~−3.2。全部 wm 爛（−5~−15）=SM 低側教材非可用解,
   但「低側導航」從零資料到有據點——v36 吃進後 SM 首次見過 gap 區物理。
3. 三首航驗收：**dyn-frac 0.4 生效**（血統根 13→**9% 新低**）;誤差錨=有效資料泵（子代直取 gap 區）;
   碎片 init=SM 新盲區探測器（randf 樣本 |Δ| 20-23 進誤差錨 top）。
4. rad 頭前瞻 **+0.621（p=.018 史上最高）**——續鍵;M 前瞻 wm +0.361/oob +0.327。
5. ⚠ 恆溫器連兩批觸發（批內 NN 32/32）→ R30 採納配額加碼;**可用帶外零推進連 6 批**
   →§1 回報線行動=R30 搭載**窮舉公證**（歷史可用帶外 top 候選全重測,證天花板 9.0 或抓漏）。
6. 前緣增量 +1（b1 +6/b2 +7/b3 +1 放緩）;近王 7%（誤差錨吸收帶回少量王系,可接受）。

## 5. 結論 (Conclude)
**R29 假設答案：SM 反傳的「pred 達標」有多少是真的？——champ 帶（d≤25）~25% 真（3 筆三標/12
pred 達標）,d≥60 全假但可被 adversarial training 教會誠實（吹牛樣本 24→0）。**
1. **SM 可信半徑實測=d≤25**（champ 帶連兩批出真解,adv 率 90→74% 收斂中）;梯度反傳解三軸耦合
   ——未直接解,但 champ 帶三標 ×3=「SM 準度變現」通道打通（戰略換軸的首個實證）。
2. **adversarial training 循環成立**：76 筆/批的 SM 最大誤差樣本使 ikpi +4.50（史上最高）、
   遠帶吹牛歸零、rad 頭前瞻爬至 +0.621——「多樣性資料→SM 準度」的因果鏈首輪閉環。
3. **低側 gap 區破冰**=意外戰果（誤差錨/denovo/碎片 init 三路踏進 −8~−2 無人區）;
   margin 王 **+0.56**（O 臂冷支,+0.06 大跳三軸均衡,24hr 冷支連兩王）。
4. 制度資產：五軸 KPI 面板/反馬太四機制/雙口徑互異度/帶別 adv 表——全部工具化並經三批實戰。

## 6. 後續決策 (Next)
- **R30=SM 準度輪 2＋低側據點擴張**（宣告制接棒）：G 臂續跑（champ 主力+碎片帶）;
  **低側資料泵升級**=gap 區 8+ 據點的鄰域變異（新臂或 oobp 擴額）;**窮舉公證搭載**
  （可用帶外零推進連 6 批的回報線行動:歷史 top 候選重測證天花板）;恆溫器連兩觸發→配額加碼
  （G-free/D +4~8）;analysis-05 促成項=r_feed 分軸 key（select 端零成本先試）。
- lo-active 配額 ~50%（decisions 反自餵②）——gap 據點出現後正式可行。
- sm_denovo 對決設計檢討（四連敗;閒時）;帶內 0.61 的 rad 修復（修回即換王,+0.11 buffer）。

## 7. 歸檔指向 (Archive)
- 結果夾: `dataset/dedust_r29b*`;公證 `r29n1a`（margin 王 o29b2_011 3/3）;填空池 `r28g1`（沿用）。
- SM: v33→v36;kpi.csv 起錨（v35 近1.19/中0.88/遠2.77）;紀錄=records.json（wm 0.56 已更新）。
- 對比圖=assets/round-29/newking_o29b2_011.png;報告圖=report/assets/sprint48.png。
- 工具資產:sm_invert gen（四帶+凍結+碎片 init）/select --gstage/--dyn-frac/誤差錨/恆溫器/
  帶別 adv 表/雙口徑互異度/分層 held-out/密度反權重——本輪全數落地。
