...
**關鍵推論（本鏈少算一整段的理由）**：饋線 W/h = 1.1/0.508 = 2.17 → Z₀ ≈ 51Ω
（Hammerstad–Jensen 51.03Ω，含 35µm 銅厚修正 49.67Ω）≈ 埠阻抗。均勻匹配線只轉相位、
**不改 |S11|**；而資料集只用 dB(S11)（幅值）→ 22.5mm 饋線長度對主 KPI 無影響，
L1/L2 都不必建模饋線本體。饋線唯一進場的地方＝它以 1.1mm 寬接在貼片 x=5mm 邊上（見 `feed_weights`）。

⚠ **這個論證鬆掉的四種情況（2026-08-02 對抗式驗證量化，寫進來免得被當成無條件成立）**：
1. **殘餘失配漣漪**（主要）：51Ω 線接 50Ω 埠，22.5mm 在 24–32GHz 掃過數個 λ_g
   → dB(S11) 出現頻率漣漪，峰峰值隨負載而異：|Γ|=−3dB 時 0.13dB、**−10dB 時 0.52dB**、
   −20dB 時 1.96dB——**恰好在 −10dB 判準與深谷處最大**。
2. **線損**：@28GHz 介電 0.230 + 導體 0.152 dB → 來回 0.76dB（含粗糙度 0.8–1.3dB）。
   純常數偏移 → **每頻點仿射校準吃得掉、不傷 rank**。但模型對斷路圖給 0.00dB 上限，
   HFSS 會給 ~−1dB。
3. **色散**：只改相位不改幅值 → 無影響。
4. **饋線本體輻射 + 有限地平面繞射**：完全沒建模。22.5mm ≈ 3.5λ_g、板 35×20mm、
   `ApplyInfiniteGP=False`。對 |S11| 影響小，**對 boresight Gain 影響大**
   → 本鏈 Gain 半邊的物理基礎比 S11 半邊薄，記帳時要分開講。
"""
import numpy as np
