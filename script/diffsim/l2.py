...
SOLVERS = {
    ...
    #! ⚠⚠ **2026-08-04 06:0x：`l3fld` 暫時退出出貨值，改回 `l3fl`。**
    #  修完兩個實作 bug（反對角零活化、A 項求積點）之後，**ρ 暴漲而選批命中率崩掉**：
    #    fit clean   `l3fl` ρ +0.4922 命中 **35.0%** → `l3fld` ρ **+0.9114** 命中 **21.7%**
    #    eval clean  `l3fl` ρ +0.3969 命中 **36.7%** → `l3fld` ρ **+0.7153** 命中 **13.3%**
    #  ——本輪第九次「指標漲、用途跌」（analysis-10 §49.5）。
    #  ⚠ 而且那兩個修正是**一起下的**，其中 A 項四點平均把自項從 `G(0)` 變成
    #  `¼[2G(0)+2G(dx)]`（換了一種離散化、連 `l3fl` 都動到）⇒ **目前無法歸因**。
    #  2×2 消融（`a_quad` 旗標）進行中；**在歸因清楚之前，出貨走 `l3fl`。**
    #  ⇒ 物理上 `l3fld` 更對（HFSS 的對角確實導通、鏡像不變式也只有它守得住），
    #    但「更對的物理」目前沒有轉成「更好的選批」——這件事本身要先弄懂。
