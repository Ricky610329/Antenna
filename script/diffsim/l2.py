...
SOLVERS = {
    ...
    #? ★★★ **出貨值（2026-08-04，§51.5 歸因完成後定案）**。
    #  §49.5 的「ρ 暴漲而命中率崩掉」已用 `a_quad` 2×2 消融歸因完畢：
    #  **不是對角的錯，是 `a_quad=4` 的錯**——`a_quad=1` 的 `l3fld` 是全場最好的
    #  （fit clean 命中 **55.0%**、ρ **+0.8918**；評估池 41.7%、負片 83.3%）。
    #! ⚠ 但代價要講明：`a_quad=1` 有一個**已量化的數值錯誤**——直接數值積分證實
    #  「對角 × 軸向共用 `cell_a`」的互耦**高估 69%**（§51.1；`l3fl` 裡 x–y 的
    #  `same`=0 把它歸零 ⇒ 從不發生，**加了對角才會**）。
    #  ⇒ **假說（§51.4）**：互耦被高估 = 對角路徑等效被加了串聯阻抗 ⇒ 橋變弱
    #    —— **而真實的橋本來就弱（10µm 頸縮）** ⇒ 這個數值錯誤可能剛好在
    #    模擬我沒建模的物理。決定性檢驗＝`a_quad=4` + 顯式頸縮阻抗（掃 25–500 pH）
    #    能不能重現 55%。**在檢驗完成前，不要宣稱「對角物理是對的」，只能說「排序有用」。**
    #? ★★ 對角連通（analysis-10 §45/§47）：
    ...
    #  修完反對角零活化之後（§49/§51.3，同批 fit clean n=680，四組態同表比）：
    #    `l3fl`  `a_quad=1` ρ +0.4585 命中 26.7%  ｜ `a_quad=4`+b=0.315 ρ +0.5046 命中 31.7%
    #    `l3fld` `a_quad=1` ρ **+0.8918** 命中 **55.0%** ｜ `a_quad=4`+b=0.315 ρ +0.8696 命中 15.0%
    #  ⇒ **無對角時「更準的求積」如預期更好（三指標全同向）；有對角時它把 55.0% 打成 15.0%**
    #    ⇒ 交互作用專屬於「準求積 × 對角基底」，見上方假說。
