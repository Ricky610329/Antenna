...
def test_hfss_setup_whitelist_is_port_scoped():
    """幾何鍵分域（鐵則 7）：`slot_spec` 只有 dual 模擬器有——給錯域要顯性擋掉，不能靜默吞
    （吞掉＝整夾用預設幾何白燒）。`pixel_count`（R69 dual 開 50×50）與 `diag_bridge_w`
    （2026-08-13 菱形橋移植進 dual）自此兩域皆收；鍵是否真為建構參數由
    test_hfss_setup_keys_are_real_simulator_params 對帳。"""
    ...
    assert {"pixel_count", "diag_bridge_w"} <= _hfss_setup_keys("dual") & _hfss_setup_keys("single")
