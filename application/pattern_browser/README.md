...
- **總攬**:統計卡(含曲線覆蓋)+n8/ndiag/wm 分布;篩選;四種呈現切換——**表格**(排序欄)/**縮圖牆**(96px,hover 浮層指標)/**圖卡**(縮圖+S11/Gain 小圖+rad 迷你極座標 φ0/φ90,當頁批抓曲線)/**散點**(X-Y 軸自選 wm·lo·rad·ndiag·n8·total;點=進詳情、拖框=送比對/建群組、Shift+拖=累加)。
- **詳情**:大圖(疊菱形站點)、全指標、S11/Gain 曲線(紅虛線=門檻、金底=26.5–29.5GHz 頻帶)、rad 極座標 φ0/φ90(全圓 ±180°、主波束朝上背瓣在下;金扇=±45° 窗、橘虛線=窗界、紅虛圈=G0−3dB、每環 5dB)、消融=wm 數字卡+**原始/菱形/挖空曲線疊圖**(有才畫)、Hamming 找鄰居。
- **比對**:2–4 筆,常駐縮圖條(外框色=線色,點進詳情)+分頁籤=**全部(預設,同一頁看完)**/Pattern(XOR)/S11 疊圖/Gain 疊圖/rad 極座標疊圖;曲線 hover 有 crosshair+數值。
